...
class RecModel(nn.Module):
    ...
    # === 新增方法：用于提取字体风格特征 ===
    def extract_style_feature(self, x):
        """
        提取用于控制字体的 2D 风格特征 (保留空间结构和笔画纹理)
        输入 x: [B, 3, H, W] 的参考书法图片
        输出: [B, C, H', W'] 的骨干网络特征图 (PP-OCRv3 中 C=512)
        """
        # 仅通过 backbone 进行前向传播，跳过 neck 和 head
        x = self.backbone(x)
        return x
